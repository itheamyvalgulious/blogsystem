...
import { isPathInside } from "./content-node.js";
import {
  buildDefaultThemeChromeCss,
  buildDefaultThemeProseCss,
  type ThemeColorPalette
} from "./theme-group-defaults.js";
import {
...
} from "./theme-groups.js";
import { toPosixPath } from "./utils.js";

/**
 * Thrown when a theme group config (theme.json) fails schema validation. The
 * server maps this to its ConfigValidationError so the admin API keeps its
 * historical 500-with-details shape; the static site generator surfaces it as
 * a plain build failure.
 */
export class ThemeGroupConfigValidationError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "ThemeGroupConfigValidationError";
  }
}
...
function getThemeGroupDirectory(configRoot: string, groupId: string) {
  ...
  if (!isPathInside(themeRoot, groupDirectory)) {
...
function resolveThemeAssetPath(configRoot: string, groupId: string, fileName: string, type?: ThemeAssetType) {
  ...
  if (!isPathInside(resolved.groupDirectory, absolutePath)) {
...
function collectThemeAssetConfigErrors(value: unknown, index: number): string[] {
  const entryPath = `themeGroup/files/${index}`;

  if (!isPlainObject(value)) {
    return [`${entryPath} must be object`];
  }
  ...
  if (type !== "css" && type !== "js") {
    return [`${entryPath} must match exactly one schema in oneOf`];
  }

  const allowedKeys =
    type === "css" ? ["adminPreview", "colorMode", "fileName", "type"] : ["adminPreview", "fileName", "type"];

  for (const key of allowedKeys) {
    if (!(key in value)) {
      errors.push(`${entryPath} must have required property '${key}'`);
    }
  }

  if (Object.keys(value).some((key) => !allowedKeys.includes(key))) {
    errors.push(`${entryPath} must NOT have additional properties`);
  }

  if ("fileName" in value) {
    if (typeof value.fileName !== "string") {
      errors.push(`${entryPath}/fileName must be string`);
    } else if (value.fileName.length < 1) {
      errors.push(`${entryPath}/fileName must NOT have fewer than 1 characters`);
    }
  }

  if ("adminPreview" in value && typeof value.adminPreview !== "boolean") {
    errors.push(`${entryPath}/adminPreview must be boolean`);
  }

  if (type === "css" && "colorMode" in value && value.colorMode !== "light" && value.colorMode !== "dark") {
    errors.push(`${entryPath}/colorMode must be equal to one of the allowed values`);
  }
...
/**
 * Validates the parsed theme.json shape (mirrors themeGroupConfigSchema) and
 * returns the normalized config. Kept dependency-free so content-core does not
 * need ajv; messages follow the same `themeGroup<path> <detail>` convention
 * the ajv-based validator used.
 */
function validateParsedThemeGroupConfig(value: unknown) {
  const errors: string[] = [];

  if (!isPlainObject(value)) {
    errors.push("themeGroup must be object");
  } else {
    const allowedKeys = ["enable", "files", "label", "mode"];

    for (const key of ["label", "enable", "files", "mode"]) {
      if (!(key in value)) {
        errors.push(`themeGroup must have required property '${key}'`);
      }
    }

    if (Object.keys(value).some((key) => !allowedKeys.includes(key))) {
      errors.push("themeGroup must NOT have additional properties");
    }

    if ("enable" in value && typeof value.enable !== "boolean") {
      errors.push("themeGroup/enable must be boolean");
    }

    if ("label" in value) {
      if (typeof value.label !== "string") {
        errors.push("themeGroup/label must be string");
      } else if (value.label.length < 1) {
        errors.push("themeGroup/label must NOT have fewer than 1 characters");
      }
    }

    if ("mode" in value && value.mode !== "light" && value.mode !== "dark") {
      errors.push("themeGroup/mode must be equal to one of the allowed values");
    }

    if ("files" in value) {
      if (!Array.isArray(value.files)) {
        errors.push("themeGroup/files must be array");
      } else {
        for (const [index, file] of value.files.entries()) {
          errors.push(...collectThemeAssetConfigErrors(file, index));
        }
      }
    }
  }
  ...
  if (errors.length > 0) {
    throw new ThemeGroupConfigValidationError(errors.join("; "));
  }

  return normalizeThemeGroupConfig(value as Partial<ThemeGroupConfig>);
}

async function readLegacyThemePalette(configRoot: string): Promise<ThemeColorPalette> {
  const configPath = path.join(configRoot, "site-theme.atlas.json");

  try {
    const raw = await fs.readFile(configPath, "utf8");
    const parsed = JSON.parse(raw) as {
      colors?: Partial<Record<keyof ThemeColorPalette, string>>;
    };
...
async function migrateLegacyThemeGroupConfig(
  ...
  for (const rawFile of rawFiles) {
    ...
    const inferredMode =
      (isThemeColorMode(cssFile.colorMode) ? cssFile.colorMode : null) ??
